...
struct VoiceProfile: Codable, Identifiable, Equatable {
    ...
    var ready: Bool {
        ...
        return !name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty && !text.isEmpty && text.count <= AppContract.shared.limits.transcript
    ...
    static func importAudio(_ source: URL, into folder: URL = Self.folder) throws -> VoiceProfile {
        ...
        guard duration.isFinite, (AppContract.shared.limits.recordingMin...AppContract.shared.limits.recordingMax).contains(duration) else { throw AppFailure("음성은 3~30초로 선택해 주세요.") }
        let samples = try AudioFiles.samples(source)
        guard samples.allSatisfy(\.isFinite), samples.contains(where: { abs($0) > Float(AppContract.shared.limits.voicePeak) }) else { throw AppFailure("음성에 들리는 목소리가 없습니다.") }
